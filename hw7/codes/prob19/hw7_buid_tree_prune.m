function root =hw7_buid_tree_prune(train_x, train_y)
...
function node = recursive_split(node)
    ...
    node.isLeaf=false;
    [lch, rch]=split(node);
    [lch.isLeaf, lch.val]=deal(true, mode(lch.train_y));
    [rch.isLeaf, rch.val]=deal(true, mode(rch.train_y));
    [node.lch, node.rch] = deal(lch, rch);
end
